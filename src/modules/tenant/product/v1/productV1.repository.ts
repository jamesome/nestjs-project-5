...
export class ProductV1Repository {
  ...
  async create(createProductDto: CreateProductDto[]) {
    const productDto = this.productV1Repository.create(createProductDto);
    // const products = productDto.map((item) => {
    //   item.createWorker = 'create_worker_name';

    //   return item;
    // });

    return await this.productV1Repository.save(productDto);
  }
